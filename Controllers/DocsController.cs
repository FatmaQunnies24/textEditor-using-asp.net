    ...
    public class DocsController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            var applicationDbContext = from c in _context.Docs
                                       select c;
            applicationDbContext = applicationDbContext.Where(a => a.UserId == User.FindFirstValue(ClaimTypes.NameIdentifier));//User.FindFirstValue(ClaimTypes.NameIdentifier) fine id from token 8

            return View(await applicationDbContext.Include(d => d.User).ToListAsync());// استخدمت انكلود عشان اجيب معلومات اليوزر مع الملفات مش بس اجيب الملفات 
        }



        // GET: Docs/Create
        public IActionResult Create()//عشان اروح على صفحة كرييت 
        {

            return View();
        }

        [HttpPost]// عشان لما اعمل سبميت يخزنه ب الداتا بيس 
        ...
        public async Task<IActionResult> Create([Bind("Id,Title,Content,UserId")] Doc doc)
        {
            if (ModelState.IsValid)
            {
                ...
                return RedirectToAction(nameof(Index));// اعادة توجيه ل فيو ثانية 
            }
